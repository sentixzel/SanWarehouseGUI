...
// NewProductForm - конструктор формы (экспортируемая функция)
...
func (pf *ProductForm) saveProduct() {
	...
	if pf.skuEntry.Text == "" || pf.nameEntry.Text == "" {
		dialog.ShowError(fmt.Errorf("Заполните обязательные поля (SKU и Наименование)"), pf.window)
		return
	}
